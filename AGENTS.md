...
- `make test-integration` — starts the compose test DB (:5434) and runs all tests with `-count=1`; the harness migrates one template database and clones a throwaway DB per test (internal/store/storetest), so a non-compose `TEST_DB_URL` user needs the CREATEDB privilege.
