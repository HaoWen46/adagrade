...
## Ubuntu / Debian (incl. WSL2)

1. **HUMAN if you lack sudo** — base tools: `sudo apt-get update && sudo apt-get install -y build-essential git curl`
2. Go 1.26+ (apt's golang is too old — use the official tarball; pick the right arch from https://go.dev/dl/): `mkdir -p "$HOME/.local" && curl -fL https://go.dev/dl/go1.26.4.linux-amd64.tar.gz | tar -C "$HOME/.local" -xz && export PATH="$HOME/.local/go/bin:$PATH"` (persist the PATH line into `~/.profile`).
...
4. Database — pick ONE:
   - Local Postgres (no Docker): `sudo apt-get install -y postgresql` (16+ on current releases), then the "Create the database" block below with `SUDO_PG="sudo -u postgres"`.
